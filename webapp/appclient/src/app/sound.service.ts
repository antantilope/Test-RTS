...
export class SoundService {
  ...
  // Button Clicks
  private buttonClickPrimarySound: HTMLAudioElement
  private buttonClickUtilitySound: HTMLAudioElement
  ...
  // APU
  private copilotAPUActiveSound: HTMLAudioElement
  private copilotAPUOfflineSound: HTMLAudioElement
  private APUActiveLastFrame = false
  ...
  private copilotDockingCompleteSound: HTMLAudioElement
  private dockedAtStationLastFrame = false
  ...
  private ebeamChargingSound: HTMLAudioElement
  ...
  private copilotDeathChoices = [
    ...
    "copilot-death-16.mp3",
    "copilot-death-17.mp3",
    "copilot-death-18.mp3",
    "copilot-death-19.mp3",
    "copilot-death-20.mp3",
  ]
  ...
  constructor(
    ...
    // Button clicks
    this.buttonClickPrimarySound = new Audio("/static/sound/button-primary.mp3")
    this.buttonClickUtilitySound = new Audio("/static/sound/button-utility.mp3")
    ...
    // APU
    this.copilotAPUActiveSound = new Audio("/static/sound/copilot-apu-active.mp3")
    this.copilotAPUOfflineSound = new Audio("/static/sound/copilot-apu-offline.mp3")
    ...
    this.copilotDockingCompleteSound = new Audio("/static/sound/copilot-docking-complete.mp3")
    ...
    this.ebeamChargingSound = new Audio("/static/sound/ebeam-charge.mp3")
    this.ebeamChargingSound.loop = true
    this.ebeamChargingSound.volume = 0.5 // raw sound file is too loud/annoying.
  ...
  public playPrimaryButtonClickSound(): void {
    this.buttonClickPrimarySound.play()
  }

  public playUtilityButtonClickSound(): void {
    this.buttonClickUtilitySound.play()
  }
  ...
  public runSoundFXEngine() {
    ...
    // APU
    if(ship.alive && ship.apu_online && !this.APUActiveLastFrame) {
      this.copilotAPUActiveSound.play()
      this.APUActiveLastFrame = true
    } else if(ship.alive && !ship.apu_online && this.APUActiveLastFrame) {
      this.copilotAPUOfflineSound.play()
      this.APUActiveLastFrame = false
    }
    ...
    // Space station Alerts
    if(ship.alive && ship.docked_at_station && !this.dockedAtStationLastFrame) {
      this.dockedAtStationLastFrame = true
      this.copilotDockingCompleteSound.play()
    } else if(ship.alive && !ship.docked_at_station && this.dockedAtStationLastFrame) {
      this.dockedAtStationLastFrame = false
    }
    ...
    if(!ship.ebeam_charging && !this.ebeamChargingSound.paused) {
      this.ebeamChargingSound.pause()
    }
  ...
  private getDistanceBetweenCoords(
    p1: PointCoord,
    p2: PointCoord
  ): number {
    return Math.sqrt(
